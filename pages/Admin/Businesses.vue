  ...
  <div style="font-weight:700; font-size:15px;">Business</div>
                    ...
                    <v-row>
                        <v-col cols="12" sm="12" md="4">
                            <div class="metrics-card">
                                  ...
                                  <v-col  v-if="businesses.data" cols="8">
                                      <div>{{businesses.data.length > 1 ? 'Businesses' : 'Business'}}</div>
                                      <div style="font-weight:700;font-size:20px;">{{businesses.data.length}}</div>
                                  </v-col>
                              </v-row>
                            </div>
                        </v-col>
    ...
    <v-tab-item style="border-radius:20px">
        <v-card flat style="padding:30px 0;background:#F2F2F2;">
					...
					<tbody>
						<tr @click="toBusiness(item.id)" v-for="(item,index) in businesses.data" :key="index" >
							<td>{{index + 1}}</td>
							<td>{{item.app_id}}</td>
...
export default {
    ...
    computed: {
        businesses(){
            return this.$store.state.admin.businesses
        }
    },
    mounted(){
        this.$store.dispatch('admin/fetchBusinesses');
    },
    data() {
		return {
			page:1,
     ...
     methods:{
        toBusiness(id){
            this.$router.push({path:`/admin/${id}/businessview`})
        }
    }
}
</script>
